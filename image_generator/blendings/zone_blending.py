...
parameters_for_wfc = copy.deepcopy(wfc_blending_parameters)

# take only n_points
desired_parameters = ["n_points"]
params_tmp = copy.deepcopy(algorithm_instances_list[AlgorithmInstancesEnum.voronoi.value].parameters)
parameters_for_voronoi = [param for param in params_tmp if param.name in desired_parameters]

# take only hex_size, spacing
desired_parameters = ["hex_size", "spacing"]
params_tmp = copy.deepcopy(algorithm_instances_list[AlgorithmInstancesEnum.hex_pattern.value].parameters)
parameters_for_hexes = [param for param in params_tmp if param.name in desired_parameters]

# take only n_layers
desired_parameters = ["n_layers"]
...
class ZoneBlendingNamed(Blending):
    def __init__(self,
        ...
        super().__init__(name + str(zone_gen_fun),
                         ...
                         "Blending of layers where place for each algorithm is distributed based on its weight and "
                         "generated by this algorithm color zones using chosen algorithm",
                         # TODO maybe later also take description from algorithm that used
